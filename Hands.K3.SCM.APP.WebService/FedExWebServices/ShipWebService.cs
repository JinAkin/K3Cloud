...
namespace Hands.K3.SCM.APP.WebService
{
    public class ShipWebService
    {
                ...
                if (notice.Packages != null && notice.Packages.Count > 0)
                {
                    ...
                    for (int i = 0; i < notice.Packages.Count; i++)
                    {
                        if (notice.Packages[i] != null)
                        {
                            ...
                            //request.RequestedShipment.RequestedPackageLineItems[i].InsuredValue.Currency = notice.FSettleCurrId;
                            //request.RequestedShipment.RequestedPackageLineItems[i].InsuredValue.Amount = notice.orderFin.FBillAllAmount;

                            request.RequestedShipment.RequestedPackageLineItems[i].Weight = new Weight();
                            request.RequestedShipment.RequestedPackageLineItems[i].Weight.Value = Math.Round(package.Weight,1);
                            ...
                            request.RequestedShipment.RequestedPackageLineItems[i].Dimensions.Length = GetIntNo(package.Dimension.Length);
                            request.RequestedShipment.RequestedPackageLineItems[i].Dimensions.Width = GetIntNo(package.Dimension.Width);
                            request.RequestedShipment.RequestedPackageLineItems[i].Dimensions.Height = GetIntNo(package.Dimension.Height);
                            request.RequestedShipment.RequestedPackageLineItems[i].Dimensions.Units = LinearUnits.CM;

                            request.RequestedShipment.RequestedPackageLineItems[i].CustomerReferences = new CustomerReference[1] { new CustomerReference() };
                            request.RequestedShipment.RequestedPackageLineItems[i].CustomerReferences[i].CustomerReferenceType = CustomerReferenceType.CUSTOMER_REFERENCE;
                            request.RequestedShipment.RequestedPackageLineItems[i].CustomerReferences[i].Value = "testreference";
                        }
                    }
                }
            }
                ...
                if (notice.OrderEntry != null && notice.OrderEntry.Count > 0)
                {
                    ...
                    for (int i = 0; i < notice.OrderEntry.Count; i++)
                    {
                        if (notice.OrderEntry[i] != null)
                        {
                            ...
                            //request.RequestedShipment.CustomsClearanceDetail.Commodities[i].UnitPrice.Currency = CurrencyCodes.GetCurrCodes(notice.orderFin.FSettleCurrID);
                            request.RequestedShipment.CustomsClearanceDetail.Commodities[i].UnitPrice.Amount = entry.FTAXPRICE * 1000000;
                            ...
                            request.RequestedShipment.CustomsClearanceDetail.Commodities[i].Weight.Value = entry.F_HS_TotalWeight;
                            ...
                            //request.RequestedShipment.CustomsClearanceDetail.Commodities[i].CustomsValue.Amount = entry.FTAXPRICE * 100;
                            //request.RequestedShipment.CustomsClearanceDetail.Commodities[i].CustomsValue.Currency = CurrencyCodes.GetCurrCodes(notice.orderFin.FSettleCurrID);
        ...
        private static void RecordNotifications(Context ctx, ProcessShipmentReply reply)
        {
            if (reply != null)
            {
                LogUtils.WriteSynchroLog(ctx, SynchroDataType.DeliveryNoticeBill, GetNotifications(reply));
            }
            else
            {
                LogUtils.WriteSynchroLog(ctx, SynchroDataType.DeliveryNoticeBill, "Fedex无响应");
            }
        }
        ...
        private static string GetIntNo(string num)
        {
            string[] arr = null;
